#!/usr/bin/env python3
"""
Glyphdown skill-level filter (SessionStart hook).
...
def set_active_level(lvl: str) -> None:
    ...
    home = Path.home()
    glyphdown_dir = home / ".ultracos"
    glyphdown_dir.mkdir(parents=True, exist_ok=True)

    tmp_file = glyphdown_dir / (FLAG_FILENAME + ".tmp")
    flag_file = glyphdown_dir / FLAG_FILENAME
...
def main() -> int:
    try:
        ...
        if not plugin_root:
            # Fallback: assume we're in glyphdown/hooks/SessionStart
            plugin_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", ".."))

        skill_path = os.path.join(plugin_root, "glyphdown", "skills", "glyphdown-mode", "SKILL.md")
