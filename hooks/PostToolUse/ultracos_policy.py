#!/usr/bin/env python3
"""ultracos per-tool learned policy (internal-ref SIL-2).
...
POLICY_VERSION = 1
SKIP_THRESHOLD = float(os.environ.get("ULTRACOS_SKIP_THRESHOLD", "5"))
MIN_SAMPLES = int(os.environ.get("ULTRACOS_MIN_SAMPLES", "50"))
COOLDOWN_SKIPS = int(os.environ.get("ULTRACOS_COOLDOWN_SKIPS", "100"))
...
def _policy_dir() -> Path:
    """Get policy dir via ultracos_paths if available, else fallback."""
    if _PATHS_AVAILABLE:
...
def _policy_file() -> Path:
    """Get policy file via ultracos_paths if available, else fallback."""
    if _PATHS_AVAILABLE:
