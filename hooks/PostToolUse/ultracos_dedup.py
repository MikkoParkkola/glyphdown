#!/usr/bin/env python3
"""ultracos PostToolUse dedup + summarize for Read/Grep/Glob/Monitor (internal-ref).
...
Session state lives at $ULTRACOS_STATE_DIR/dedup-<session_id>.json
...
def _state_dir() -> Path:
    """Resolve dedup state directory. Tests override via ULTRACOS_STATE_DIR."""
    override = os.environ.get("ULTRACOS_STATE_DIR", "").strip()
    if override:
...
def summarize_large(text: str) -> tuple[str, bool]:
    ...
    if middle_errors:
        pieces.append(
            f"[ultracos:summarize-v1 kept={len(middle_errors)} error-lines"
        ...
        pieces.append(
            f"[ultracos:summarize-v1 hidden={hidden_lines} lines, no error pattern]"
        )
    pieces.extend(tail)
    out = "\n".join(pieces)
