#!/usr/bin/env python3
"""ultracos cache-aware compression (internal-ref SIL-5).
...
Storage: ``<ultracos_data_dir>/cache_state.json``
Schema (v1):
    {
      "version": 1,
      "prefixes": {
...
Tunables (env):
- ``ULTRACOS_CACHE_AWARE``        bool   (default FALSE; opt-in flag)
- ``ULTRACOS_CACHE_PREFIX_BYTES`` int    (default 1024; min prefix length to track)
- ``ULTRACOS_CACHE_HOT_HITS``     int    (default 2;  hits to declare hot)
- ``ULTRACOS_CACHE_TTL_SECONDS``  int    (default 7 * 86400)
- ``ULTRACOS_CACHE_MAX_ENTRIES``  int    (default 2048; LRU cap)
"""
...
def _config() -> dict:
    ...
    return {
        ...
        "enabled": _bool_env("ULTRACOS_CACHE_AWARE", False),
        "prefix_bytes": max(64, _int_env("ULTRACOS_CACHE_PREFIX_BYTES", 1024)),
        "hot_hits": max(2, _int_env("ULTRACOS_CACHE_HOT_HITS", 2)),
        "ttl_seconds": max(60, _int_env("ULTRACOS_CACHE_TTL_SECONDS", 7 * 86400)),
        "max_entries": max(16, _int_env("ULTRACOS_CACHE_MAX_ENTRIES", 2048)),
    }


# Best-effort import of ultracos_paths so we share the same data dir as the
...
try:
    import ultracos_paths as _paths  # type: ignore
