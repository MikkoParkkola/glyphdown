#!/usr/bin/env python3
"""ultracos SIL-3 schema-tag A/B effectiveness monitor (internal-ref).

The codec emits `[ultracos:compact-v1 ...]` on every compacted payload to
...
def _ab_disabled() -> bool:
    """ULTRACOS_AB_DISABLE=1 forces with-tag always."""
    raw = os.environ.get("ULTRACOS_AB_DISABLE", "").strip().lower()
...
def _monitor_auto_disabled() -> bool:
    ...
    Consulted AFTER ULTRACOS_AB_DISABLE. When set, default to no-tag.
    Fail-open: any error → False (stay in experiment).
    """
    try:
        import ultracos_ab_monitor as _mon  # local import; fail-open if absent
...
def decide_variant(session_id: Optional[str], seed: Optional[str] = None) -> str:
    ...
    Consultation order:
    1. ULTRACOS_AB_DISABLE=1  → force with-tag (operator kill switch)
