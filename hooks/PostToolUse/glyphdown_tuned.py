#!/usr/bin/env python3
"""glyphdown SIL-1 self-improving loop: codec break-even auto-tuner.
...
- GLYPHDOWN_BREAK_EVEN_TOKENS env var wins over tuned thresholds (codec-side).
- GLYPHDOWN_NO_LEARN=1 disables loading (codec-side: skip load_thresholds()).
...
- Periodic refresh: this module is cron-runnable as a one-shot CLI —
  ``*/15 * * * * python3 glyphdown_tuned.py refresh`` — so the
