#!/usr/bin/env python3
"""glyphdown schema-tag A/B effectiveness monitor (internal-ref).
...
(that's internal-ref). The hypothesis being tested: does emitting the `[glyphdown:
...
(default to no-tag). It is SEPARATE from GLYPHDOWN_AB_DISABLE (which forces
with-tag and is used to short-circuit the experiment). See `decide_variant`
in glyphdown_ab.py for the consultation order.
...
try:
    from glyphdown_paths import glyphdown_data_dir as _data_dir  # type: ignore
except Exception:  # noqa: BLE001
    def _data_dir() -> Path:
        env = os.environ.get("GLYPHDOWN_DATA_DIR")
        if env:
...
def _audit_path() -> Path:
    raw = os.environ.get("GLYPHDOWN_AUDIT_FILE")
    if raw:
...
def _rollup_path() -> Path:
    raw = os.environ.get("GLYPHDOWN_AB_ROLLUP_FILE")
    if raw:
        return Path(raw).expanduser()
...
def _disable_flag_path() -> Path:
    raw = os.environ.get("GLYPHDOWN_AB_DISABLE_FLAG")
    if raw:
        return Path(raw).expanduser()
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    p = argparse.ArgumentParser(
        description="glyphdown schema-tag A/B effectiveness monitor (internal-ref)",
    )
