#!/usr/bin/env python3
"""UltraCoS PreCompact hook — instruct the compaction summarizer to
emit ULTRACOS-MAX dense form, prefixed with a cache-stable preamble.
...
compaction summary in ULTRACOS-MAX form starting with a fixed byte
...
PREAMBLE = (
    "## ULTRACOS COMPACTED CONTEXT v1\n"
    "### Session-invariants\n"
    "- format: ULTRACOS-MAX dense form\n"
    "- schema: goal | state | delta | decisions | files | validation"
    " | blockers | next | restore\n"
    "- preamble-hash: ultracos-compact-cache-stable-v1\n"
...
Produce the compaction summary in ULTRACOS-MAX dense form, NOT prose.
...
def _ultracos_data_dir() -> Path:
    env_dir = os.environ.get("ULTRACOS_DATA_DIR")
    if env_dir:
...
def main() -> int:
    ...
    try:
        ...
        print(f"ultracos-compact-instructions: {e}", file=sys.stderr)
