#!/usr/bin/env python3
"""UltraCoS-Stats UserPromptSubmit hook.

Detects /ultracos-stats trigger (including "ultracos stats", "show ultracos stats")
...
def read_audit_jsonl() -> list[dict]:
    ...
    try:
        ...
        print(f"ultracos-stats-handler: failed to read audit.jsonl: {e}", file=sys.stderr)
...
def format_table(stats: dict, share: bool = False) -> str:
    ...
    if share:
        ...
        return f"UltraCoS saved {session:,}T this session, {lifetime:,}T lifetime (~${usd:.4f})"
...
def main() -> int:
    try:
        ...
        try:
            ...
            suffix_path.write_text(f"[UltraCoS: {stats['lifetime_tokens_saved']:,}T saved]\n")
        except Exception as e:
            print(f"ultracos-stats-handler: failed to write statusline-suffix: {e}", file=sys.stderr)
        ...
        print(f"ultracos-stats-handler: {e}", file=sys.stderr)
