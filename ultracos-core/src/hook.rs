...
//!   so it would be a cold-start no-op). Set ULTRACOS_RUST=0 for python.
...
/// internal-ref anchor guard toggle (ULTRACOS_ANCHOR_GUARD, default ON — matches
/// python ANCHOR_GUARD_ENABLED). Only "0"/"false"/"no"/"off" disable it.
fn anchor_guard_enabled() -> bool {
    match std::env::var("ULTRACOS_ANCHOR_GUARD") {
...
fn run() -> anyhow::Result<String> {
        ...
        .and_then(Value::as_array_mut)
    {
        for item in items.iter_mut() {
            ...
            // SIL-5 cache-hot bypass (only when ULTRACOS_CACHE_AWARE enabled).
