...
impl CadenceParams {
    fn from_env() -> Self {
        Self {
            sessions_per_day: env_f64("ULTRACOS_SESSIONS_PER_DAY", 3.0),
            compactions_per_session: env_f64("ULTRACOS_COMPACTIONS_PER_SESSION", 4.0),
            days_per_year: env_f64("ULTRACOS_DAYS_PER_YEAR", 365.0),
        }
    }
}
...
mod tests {
    ...
    fn cadence_env_vars_scale_savings_linearly() {
        ...
        unsafe {
            std::env::remove_var("ULTRACOS_SESSIONS_PER_DAY");
            std::env::remove_var("ULTRACOS_COMPACTIONS_PER_SESSION");
            std::env::remove_var("ULTRACOS_DAYS_PER_YEAR");
        }
        let base = scan_all_sessions(tmp.path()).unwrap();
        unsafe {
            std::env::set_var("ULTRACOS_SESSIONS_PER_DAY", "6");
        }
