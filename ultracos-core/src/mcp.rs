...
//! exposes the ULTRACOS codec to ANY MCP-speaking client (Cursor, Cline, Zed,
...
fn tool_specs() -> Value {
    json!([
        {
            "name": "ultracos_compress",
            "description": "Compress prose to the ULTRACOS-L1 dense register (lossless; \
                            unrecognized text passes through). The target LLM reads the \
                            dense form natively. Reverse with ultracos_expand.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        {
            "name": "ultracos_expand",
            "description": "Expand ULTRACOS-L1 dense text back to prose (exact inverse of \
                            ultracos_compress).",
            "inputSchema": {
                "type": "object",
                "properties": {
...
fn handle_tools_call(id: Value, req: &Value) -> Value {
    ...
    match name {
        "ultracos_compress" => match s("text") {
            Some(t) => rpc_ok(id, tool_text(codec::compress(t))),
            None => rpc_error(id, -32602, "ultracos_compress: missing required 'text'"),
        },
        "ultracos_expand" => match s("text") {
            Some(t) => rpc_ok(id, tool_text(codec::expand(t))),
            None => rpc_error(id, -32602, "ultracos_expand: missing required 'text'"),
        },
        "ultracos_compress_config" => match s("text") {
            Some(t) => {
            ...
            None => rpc_error(id, -32602, "ultracos_compress_config: missing required 'text'"),
        },
        "ultracos_retrieve" => {
            let (Some(session), Some(rid)) = (s("session"), s("id")) else {
                return rpc_error(
                    id,
                    -32602,
                    "ultracos_retrieve: missing required 'session' and/or 'id'",
                );
            };
            let range = s("range");
...
mod tests {
    ...
    fn initialize_echoes_client_protocol_version() {
        ...
        assert_eq!(r["result"]["serverInfo"]["name"], "ultracos-core");
    ...
    fn compress_roundtrips_via_tools_call() {
        ...
        let call = json!({
            ...
            "params":{"name":"ultracos_compress","arguments":{"text":probe}}
        });
        ...
        let back = json!({
            ...
            "params":{"name":"ultracos_expand","arguments":{"text":dense}}
        });
    ...
    fn compress_config_reports_savings_json() {
        let call = json!({
            ...
            "params":{"name":"ultracos_compress_config","arguments":{"text":"decision and implementation"}}
        });
        let r = handle(&call).unwrap();
    ...
    fn retrieve_miss_is_iserror_not_protocol_error() {
        let call = json!({
            ...
            "params":{"name":"ultracos_retrieve","arguments":{"session":"nope","id":"deadbeef"}}
        });
        let r = handle(&call).unwrap();
    ...
    fn missing_required_arg_is_protocol_error() {
        let call = json!({
            ...
            "params":{"name":"ultracos_compress","arguments":{}}
        });
        let r = handle(&call).unwrap();
    ...
    fn unknown_tool_is_invalid_params() {
        let call = json!({
            ...
            "params":{"name":"ultracos_nonexistent","arguments":{}}
        });
        let r = handle(&call).unwrap();
        assert_eq!(r["error"]["code"], -32602);
    }
