...
//! black hole, and `ultracos_tuned` (SIL-1) has no data to fit per-tool
...
//!   2. SESSION ~/.claude/data/ultracos/audit-<session>.jsonl  (internal-ref)
//! NB the two roots differ: global honors ULTRACOS_DATA_DIR / ~/.ultracos;
//! per-session is always under HOME/.claude/data/ultracos (matching python).
//!
...
pub fn write_row(session_id: &str, row: &Value) {
    ...
    // 2) per-session (HOME/.claude/data/ultracos/audit-<safe>.jsonl)
