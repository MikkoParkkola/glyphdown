...
//! every real record. Opt-in (`GLYPHDOWN_JSON_SAMPLE`), default OFF.
...
fn enabled() -> bool {
    matches!(
        std::env::var("GLYPHDOWN_JSON_SAMPLE")
            .unwrap_or_default()
            .trim()
            .to_ascii_lowercase()
            .as_str(),
...
pub fn sample_json(session: &str, text: &str) -> Option<Sampled> {
    ...
    let marker = serde_json::json!({
        "__glyphdown_sampled__": {
            "total": arr.len(),
            "kept": kept.len(),
            "schema": schema,
            "anomalies_preserved": anomalies.len(),
            "retrieve": format!("glyphdown-core retrieve {session} {id}"),
        }
    });
...
mod tests {
    ...
    fn on(name: &str) {
        let dir = std::env::temp_dir().join(format!("glyphdown-jsonsample-test-{name}"));
        let _ = std::fs::remove_dir_all(&dir);
        std::fs::create_dir_all(&dir).unwrap();
        // SAFETY: serialized via the shared lock below.
        unsafe {
            std::env::set_var("GLYPHDOWN_REWIND_DIR", &dir);
            std::env::set_var("GLYPHDOWN_JSON_SAMPLE", "1");
        }
    }
    fn off() {
        unsafe {
            std::env::remove_var("GLYPHDOWN_REWIND_DIR");
            std::env::remove_var("GLYPHDOWN_JSON_SAMPLE");
        }
    }
    ...
    fn off_by_default() {
        ...
        unsafe { std::env::remove_var("GLYPHDOWN_JSON_SAMPLE") };
