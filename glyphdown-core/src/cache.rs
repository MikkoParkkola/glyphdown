//! cache — cache-hot compression bypass, Rust port of glyphdown_cache.py.
//!
...
//!
//! Default OFF (`GLYPHDOWN_CACHE_AWARE`), so in the flipped-default rust path
...
fn config() -> Config {
    Config {
        enabled: bool_env("GLYPHDOWN_CACHE_AWARE", false),
        prefix_bytes: int_env("GLYPHDOWN_CACHE_PREFIX_BYTES", 1024).max(64) as usize,
        hot_hits: int_env("GLYPHDOWN_CACHE_HOT_HITS", 2).max(2),
        ttl_seconds: int_env("GLYPHDOWN_CACHE_TTL_SECONDS", 7 * 86400).max(60) as f64,
        max_entries: int_env("GLYPHDOWN_CACHE_MAX_ENTRIES", 2048).max(16) as usize,
    }
}
...
mod tests {
    ...
    fn disabled_is_zero_io_noop() {
        // GLYPHDOWN_CACHE_AWARE unset -> never bypass, no panic.
